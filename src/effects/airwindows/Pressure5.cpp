...
void Pressure5::setProgramName(char *name) {
  vst_strncpy(_programName, name, kVstMaxProgNameLen);
}
void Pressure5::getProgramName(char *name) {
...
void Pressure5::getParameterName(VstInt32 index, char *text) {
  switch (index) {
  case kParamA:
    vst_strncpy(text, "Pressre", kVstMaxParamStrLen);
    break;
  case kParamB:
    vst_strncpy(text, "Speed", kVstMaxParamStrLen);
    break;
  case kParamC:
    vst_strncpy(text, "Mewines", kVstMaxParamStrLen);
    break;
  case kParamD:
    vst_strncpy(text, "PawClaw", kVstMaxParamStrLen);
    break;
  case kParamE:
...
bool Pressure5::getEffectName(char *name) {
  vst_strncpy(name, "Pressure5", kVstMaxProductStrLen);
  return true;
}
...
bool Pressure5::getProductString(char *text) {
  vst_strncpy(text, "airwindows Pressure5", kVstMaxProductStrLen);
  return true;
}
