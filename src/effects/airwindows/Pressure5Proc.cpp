...
void Pressure5::processReplacing(float **inputs, float **outputs,
  ...
  while (--sampleFrames >= 0) {
    ...
    if (fixA[fix_freq] < 0.4999) {
      ...
      fixA[fix_sL1] =
          (inputSampleL * fixA[fix_a1]) - (temp * fixA[fix_b1]) + fixA[fix_sL2];
      fixA[fix_sL2] = (inputSampleL * fixA[fix_a2]) - (temp * fixA[fix_b2]);
      inputSampleL = temp;
      temp = (inputSampleR * fixA[fix_a0]) + fixA[fix_sR1];
      fixA[fix_sR1] =
          (inputSampleR * fixA[fix_a1]) - (temp * fixA[fix_b1]) + fixA[fix_sR2];
    ...
    // we will take the greater of either channel and just use that, then apply
    // the result to both stereo channels.
    ...
    if (flip) {
      ...
      coefficient =
          (coefficient * mewiness) + (muCoefficientA * (1.0 - mewiness));
      ...
      coefficient =
          (coefficient * mewiness) + (muCoefficientB * (1.0 - mewiness));
      inputSampleL *= coefficient;
      inputSampleR *= coefficient;
    }
    ...
    if (fixB[fix_freq] < 0.49999) {
      ...
      fixB[fix_sL1] =
          (inputSampleL * fixB[fix_a1]) - (temp * fixB[fix_b1]) + fixB[fix_sL2];
      fixB[fix_sL2] = (inputSampleL * fixB[fix_a2]) - (temp * fixB[fix_b2]);
      inputSampleL = temp;
      temp = (inputSampleR * fixB[fix_a0]) + fixB[fix_sR1];
      fixB[fix_sR1] =
          (inputSampleR * fixB[fix_a1]) - (temp * fixB[fix_b1]) + fixB[fix_sR2];
      fixB[fix_sR2] = (inputSampleR * fixB[fix_a2]) - (temp * fixB[fix_b2]);
      inputSampleR = temp; // fixed biquad filtering ultrasonics between
                           // Pressure and ClipOnly
    }
    ...
    // set up for fiddling with mewiness. Largest amount of slew in any
    // direction

    // begin ClipOnly2 stereo as a little, compressed chunk that can be dropped
    // into code
    if (inputSampleL > 4.0)
      inputSampleL = 4.0;
    if (inputSampleL < -4.0)
    ...
    inputSampleL =
        lastSampleL; // Latency is however many samples equals one 44.1k sample
    ...
    inputSampleR =
        lastSampleR; // Latency is however many samples equals one 44.1k sample
    for (int x = spacing; x > 0; x--)
      intermediateR[x - 1] = intermediateR[x];
    lastSampleR = intermediateR[0]; // run a little buffer to handle this
    // end ClipOnly2 stereo as a little, compressed chunk that can be dropped
    // into code final clip runs AFTER the Dry/Wet. It serves as a safety clip
    // even if you're not full wet
...
void Pressure5::processDoubleReplacing(double **inputs, double **outputs,
                                       VstInt32 sampleFrames) {
  double *in1 = inputs[0];
  double *in2 = inputs[1];
