...
void Console7Channel::processReplacing(float **inputs, float **outputs,
  ...
  double inputgain = 1.0 + (A * 1.0); // 0.0 -> Unity, 1.0 -> 2x Gain (+6dB)
  ...
  // louder even as it saturates and loses a bit of peak energy. Console7Channel
  // channels go to 12! (.272,etc) Neutral gain through the whole system with a
  // full scale sine ia 0.772 on the gain knob
  ...
  while (--sampleFrames >= 0) {
    ...
    double outSampleL = biquadA[2] * inputSampleL + biquadA[3] * biquadA[7] +
    ...
    double outSampleR = biquadA[2] * inputSampleR + biquadA[3] * biquadA[11] +
    ...
    // this trim control cuts back extra hard because we will amplify after the
    // distortion that will shift the distortion/antidistortion curve, in order
    // to make faded settings slightly 'expanded' and fall back in the
    // soundstage, subtly
    ...
    if (inputSampleL < -1.097)
      inputSampleL = -1.097;
    inputSampleL = ((sin(inputSampleL * fabs(inputSampleL)) /
                     ((fabs(inputSampleL) == 0.0) ? 1 : fabs(inputSampleL))) *
                    0.8) +
    ...
    if (inputSampleR < -1.097)
      inputSampleR = -1.097;
    inputSampleR = ((sin(inputSampleR * fabs(inputSampleR)) /
                     ((fabs(inputSampleR) == 0.0) ? 1 : fabs(inputSampleR))) *
                    0.8) +
...
void Console7Channel::processDoubleReplacing(double **inputs, double **outputs,
                                             VstInt32 sampleFrames) {
  double *in1 = inputs[0];
  double *in2 = inputs[1];
