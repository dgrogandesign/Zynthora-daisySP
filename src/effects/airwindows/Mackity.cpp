...
void Mackity::setProgramName(char *name) {
  vst_strncpy(_programName, name, kVstMaxProgNameLen);
}
void Mackity::getProgramName(char *name) {
...
void Mackity::getParameterName(VstInt32 index, char *text) {
  switch (index) {
  case kParamA:
    vst_strncpy(text, "In Trim", kVstMaxParamStrLen);
    break;
  case kParamB:
    vst_strncpy(text, "Out Pad", kVstMaxParamStrLen);
    break;
  default:
    break; // unknown parameter, shouldn't happen!
...
bool Mackity::getEffectName(char *name) {
  vst_strncpy(name, "Mackity", kVstMaxProductStrLen);
  return true;
}
...
bool Mackity::getProductString(char *text) {
  vst_strncpy(text, "airwindows Mackity", kVstMaxProductStrLen);
  return true;
}
