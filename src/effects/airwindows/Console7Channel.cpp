...
// new Console7Channel(audioMaster);}
...
void Console7Channel::setProgramName(char *name) {
  vst_strncpy(_programName, name, kVstMaxProgNameLen);
}
void Console7Channel::getProgramName(char *name) {
...
VstInt32 Console7Channel::setChunk(void *data, VstInt32 byteSize,
                                   bool isPreset) {
...
void Console7Channel::getParameterName(VstInt32 index, char *text) {
  switch (index) {
  case kParamA:
    vst_strncpy(text, "Fader", kVstMaxParamStrLen);
    break;
  default:
    break; // unknown parameter, shouldn't happen!
...
bool Console7Channel::getEffectName(char *name) {
  vst_strncpy(name, "Console7Channel", kVstMaxProductStrLen);
  return true;
}
...
bool Console7Channel::getProductString(char *text) {
  vst_strncpy(text, "airwindows Console7Channel", kVstMaxProductStrLen);
  return true;
}
